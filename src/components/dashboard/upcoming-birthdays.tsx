...
export function UpcomingBirthdays() {
  ...
  return (
    <div>
      ...
      <CollapsibleSection
        storageKey="dashboard-birthdays-expanded"
        previewCount={3}
        items={data.birthdays}
        showMoreLabel={(hidden) =>
          `Show all ${hidden + 3} upcoming birthdays`
        }
        showLessLabel="Show fewer"
        className="space-y-1"
        renderItem={(bday) => <BirthdayRow key={bday.id} bday={bday} />}
      />
    </div>
  );
}

function BirthdayRow({ bday }: { bday: UpcomingBirthday }) {
  ...
  return (
    <Link
      ...
      onMouseLeave={(e) => {
        if (!bday.isToday) e.currentTarget.style.backgroundColor = "";
      }}
    >
