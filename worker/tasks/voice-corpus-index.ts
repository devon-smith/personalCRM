...
const voiceCorpusIndex: Task = async (rawPayload, helpers) => {
  ...
  try {
    ...
    for (const userId of userIds) {
      ...
      // M0.x.13 — WhatsApp pass. Email and WA index independently so a
      // Gmail outage doesn't block WA, and vice versa. The aggregator at
      // the end (in profile.ts) unifies whatever VoiceExample rows exist.
      const waSummary = await indexWhatsAppForUser(
        prisma,
        userId,
        helpers,
      ...
      helpers.logger.info(
        `voice-corpus[whatsapp]: user=${userId} indexed=${waSummary.examplesWritten} ` +
          `skipped=${waSummary.skipped} embedFailed=${waSummary.embedFailures}`,
      );
    }
  } finally {
...
/**
 * M0.x.13 — WhatsApp outbound voice indexing.
 *
 * Walks `Interaction` rows where channel="whatsapp", direction="OUTBOUND",
 * fullBody is set, and no VoiceExample row exists yet. Runs the same
 * feature extraction as email (minus per-user signature stripping —
 * WhatsApp messages don't carry email-style signature blocks). Classifies
 * the recipient via contactId since WA outbound has no recipient email.
 * Embeds, writes a VoiceExample with channel="whatsapp" and
 * interactionId=X (emailMessageId/gmailId left null).
 *
 * Independent from the email pass: shares cache, classifier, embedding
 * client, but runs as a separate query so a Gmail outage doesn't kill
 * WA indexing and vice versa.
 */
const WA_PER_RUN = 200;

async function indexWhatsAppForUser(
  prisma: PrismaClient,
  userId: string,
  helpers: { logger: { info: (msg: string) => void; debug: (msg: string) => void } },
  rebuild: boolean,
  ...
  // Candidate set: outbound WA interactions with full body and no
  // existing VoiceExample. We don't backfill bodies — the sync route
  // already persists fullBody at ingest time.
  ...
  if (candidates.length === 0) {
    helpers.logger.debug(`voice-corpus[whatsapp]: nothing to index for ${userId}`);
    return summary;
  }

  interface PreparedWA {
  ...
  const prepared: PreparedWA[] = [];
  for (const c of candidates) {
    const body = c.fullBody ?? "";
    if (!body) {
      summary.skipped++;
      continue;
    }
    // WA messages have no per-user signature lines — pass [].
  ...
  // Embedding step — identical pattern to the email pass.
  const EMBED_BATCH_SIZE = Math.min(32, MAX_BATCH_SIZE);
  const RETRY_DELAY_MS = 8000;
  const embeddings = new Map<string, number[]>(); // interactionId → vector
  if (process.env.VOYAGE_API_KEY && prepared.length > 0) {
    for (let i = 0; i < prepared.length; i += EMBED_BATCH_SIZE) {
      const slice = prepared.slice(i, i + EMBED_BATCH_SIZE);
      const texts = slice.map((p) => truncateForEmbedding(p.cleanedBody));
      ...
      for (let attempt = 0; attempt < 2 && !succeeded; attempt++) {
        try {
          ...
          const msg = err instanceof Error ? err.message : String(err);
          helpers.logger.info(
            `voice-corpus[whatsapp]: voyage ${retryable ? "retryable" : "terminal"} ` +
              `error on batch ${i}-${i + slice.length} attempt ${attempt + 1}: ${msg}`,
          );
          if (!retryable) break;
          await new Promise((r) => setTimeout(r, RETRY_DELAY_MS));
  ...
  for (const p of prepared) {
    const embedding = embeddings.get(p.interactionId);
    const baseData = {
      userId,
      channel: "whatsapp",
      relationshipType: p.relationshipType,
    ...
    await prisma.voiceExample.upsert({
      where: { interactionId: p.interactionId },
      create: { interactionId: p.interactionId, ...baseData },
      update: baseData,
    });
    if (embedding) {
      await prisma.$executeRaw`
  ...
  // Re-aggregate the profile snapshot so /settings/voice picks up the
  // new WA examples on next load. Cheap — reads VoiceExample rows once.
  await aggregateVoiceProfile(prisma, userId);
