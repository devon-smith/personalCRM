...
- Draft refine SSE still has no server-side duplicate-request guard. Add either a short per-draft/request in-flight lock that returns a retryable duplicate response, or client idempotency keys if replaying a stream is required.
